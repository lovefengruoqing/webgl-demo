...
const render = (canvas: HTMLCanvasElement) => {
  ...
  const innerLimitUniformLocation = gl.getUniformLocation(program, 'u_innerLimit');
  const outerLimitUniformLocation = gl.getUniformLocation(program, 'u_outerLimit');
  ...
  const drawScene = () => {
    ...
    const lightPosition: [number, number, number] = [40, 60, 120];
    gl.uniform3fv(lightWorldPositionUniformLocation, lightPosition);
    ...
    mat4.lookAt(lmat, lightPosition, target, up);
    mat4.invert(lmat, lmat);
    mat4.multiply(lmat, mat4.rotateX(mat4.create(), mat4.create(), config.lightRotationX), lmat);
    mat4.multiply(lmat, mat4.rotateY(mat4.create(), mat4.create(), config.lightRotationY), lmat);
    const lightDirection = [-lmat[8], -lmat[9], -lmat[10]];
    gl.uniform3fv(lightDirectionUniformLocation, lightDirection);
    gl.uniform1f(innerLimitUniformLocation, config.innerLimit);
    gl.uniform1f(outerLimitUniformLocation, config.outerLimit);
  ...
  gui.add(config, 'innerLimit', 0, Math.PI, 0.001).onChange(() => {
    drawScene();
  });
  gui.add(config, 'outerLimit', 0, Math.PI, 0.001).onChange(() => {
    drawScene();
  });
  gui.add(config, 'lightRotationX', -Math.PI, Math.PI, 0.001).onChange(() => {
    drawScene();
  });
  gui.add(config, 'lightRotationY', -Math.PI, Math.PI, 0.001).onChange(() => {
    drawScene();
  });
};
